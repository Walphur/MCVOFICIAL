...
(function (w) {
    ...
    var DICT = {
        es: {
            ...
            "players.bulkStaffHint": "¿Todavía no podés usar el bot en Discord? El <strong>staff</strong> puede cargar la lista <strong>sin Hexaytron ni permisos de servidor</strong>: <a href=\"login.html\">login admin</a> → pestaña <strong>Torneos</strong> → bloque «Lista wipe» → pegá los SteamID64 (17 dígitos) y pulsá <em>Importar / fusionar</em>. También existe la variable de entorno <code>MCV_WIPE_IMPORT_STEAMS</code> en el servidor (lista al arrancar).",
        ...
        en: {
            ...
            "players.bulkStaffHint": "Can’t use the Discord bot yet? <strong>Staff</strong> can load the roster <strong>without Hexaytron or server admin</strong>: <a href=\"login.html\">admin login</a> → <strong>Tournaments</strong> tab → “Wipe list” block → paste SteamID64 values (17 digits) and click <em>Import / merge</em>. You can also set the <code>MCV_WIPE_IMPORT_STEAMS</code> env var on the server (list applied on boot).",
